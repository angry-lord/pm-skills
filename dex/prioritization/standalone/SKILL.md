---
name: prioritization
description: Prioritize a list (features, initiatives, tasks) with the right method. Guides context, suggests RICE/MoSCoW/Value vs Effort/ICE/Weighted Scoring/Kano/Cost of Delay/WSJF/Opportunity Scoring/Story Mapping/Stack Ranking, runs the method, and returns a documented order. Use when the user wants to prioritize something or choose in what order to do things.
---

# Prioritization (standalone skill)

Use this skill in any assistant that supports Agent Skills (e.g. Cursor, Manus). No dependency on a specific file system or vault.
...
**Output format:** Markdown in the chat so the user can copy it. If the assistant can save files, the user may ask to save to a path; otherwise the user saves the markdown themselves.
...
For formulas and detailed when-to-use, the user can attach **Methods_Reference.md** in the same chat.
